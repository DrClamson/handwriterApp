...
4. Assign each graph in the document to the nearest cluster in the clustering template. 
...
# Writership Analysis
The handwriter package used the statistical model and the estimated writer profile from a questioned document to calculate the posterior probability that each person wrote the questioned document. Table 3 shows the posterior probabilities of writership for each questioned document and each person of interest. Each row in the table corresponds to a person of interest and each column corresponds to a questioned document.
